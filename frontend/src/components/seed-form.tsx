	...
	const credentialsForm = useFormik({
		...
		onSubmit: async (values) => {
			try {
				...
				const { encrypted, salt, iv } = encryptMnemonic(
					mnemonic,
					values.password
				);
		...
		<Container maxWidth="sm" sx={{ py: 6 }}>
			...
			{activeStep === 2 && (
				<Paper elevation={3} sx={{ p: 4, borderRadius: 3 }}>
					...
					<Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
						Set a username and password to protect your wallet.
					</Typography>
					<form onSubmit={credentialsForm.handleSubmit}>
						<Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
							<TextField
								label="Username"
								fullWidth
								{...credentialsForm.getFieldProps("username")}
								error={credentialsForm.touched.username && Boolean(credentialsForm.errors.username)}
								helperText={credentialsForm.touched.username && credentialsForm.errors.username}
							/>
							<TextField
								label="Password"
								type="password"
